...
RISK_PCT = 1.0              # of equity, per position
MAX_OPEN = 15
# Seit dem 02.09.2026 laeuft nur noch EIN Kandidat. Diese Grenze war dafuer
# da, dass sich fuenf Kandidaten nicht gegenseitig verdraengen - jetzt wuerde
# sie still zur Hauptgrenze werden. Die Begrenzung uebernimmt MAX_PER_GROUP.
MAX_PER_CANDIDATE = 15
MAX_NEW_PER_SCAN = 5
...
MAX_PER_GROUP = 1
# Passt eine Position nicht ins freie Kapital, wird sie verkleinert statt
# verworfen. Faellt sie dabei unter diesen Anteil der Wunschgroesse, lohnt
# sie sich nicht mehr - dann doch ueberspringen.
MIN_SIZE_FRAC = 0.10

# --- VORAB ANGEMELDET, 02.09.2026 -----------------------------------------
# Kein Trade, dessen Kosten die noetige Trefferquote um mehr als diesen
# Betrag anheben.
#
# Nullpunkt bei Ziel m und Kosten c:      p = (1 + c) / (m + 1)
# Fairer Muenzwurf ohne Vorteil:          p = 1 / (m + 1)
# Nachteil allein durch Kosten:           c / (m + 1)
#
# Bei 2R-Ziel entspricht 3,33 Prozentpunkte genau der urspruenglich
# angemeldeten Grenze von 0,10R - die Anmeldung bleibt also gueltig. Bei
# groesserem Ziel faellt derselbe Spread weniger ins Gewicht, deshalb wird
# hier der Nachteil begrenzt und nicht die Kostenzahl selbst.
MAX_COST_HANDICAP_PP = 100.0 / 30.0        # 3,33 Prozentpunkte
...
def position_margin(p) -> float:
    """Gebundenes Kapital einer Position: Gegenwert geteilt durch den Hebel."""
    import datafeed
    notional = abs(float(p.get("qty") or 0)) * float(p.get("entry") or 0)
    return notional / max(1, datafeed.leverage_of(p.get("symbol", "")))


def account_state(book) -> dict:
    """Was der Bot ueber sein Konto weiss, bevor er eine Position eroeffnet."""
    equity = float(book.get("equity") or START_EQUITY)
    used = sum(position_margin(p) for p in (book.get("open") or []))
    notional = sum(abs(float(p.get("qty") or 0)) * float(p.get("entry") or 0)
                   for p in (book.get("open") or []))
    return {"equity": equity, "used_margin": used,
            "free_margin": equity - used, "notional": notional}


# -------------------------------------------------------------------- scan --
def run_once(verbose: bool = True) -> None:
    import backtest, datafeed
    # Nicht mehr forward.CANDIDATES: diese Liste ist die Vorab-Anmeldung des
    # Rueckwaertstests und bleibt als Beleg unveraendert. Was der Bot LIVE
    # handelt, steht seit dem 02.09.2026 in live_strategy.py.
    from live_strategy import LIVE as CANDIDATES, TIMEFRAME
    ...
    try:
        got = datafeed.prefetch(datafeed.ALL_SYMBOLS, TIMEFRAME, 900)
        if verbose:
    ...
    for sym in datafeed.ALL_SYMBOLS:
        try:
            cs = datafeed.fetch(sym, TIMEFRAME, 900)
        except Exception as exc:
            if verbose:
        ...
        for cid, _tf, params, _why in rotated:
            ...
            # Groesseres Ziel fuer teure Instrumente (siehe datafeed.RR_OVERRIDE)
            ...
            if dist <= 0: continue
            # Kostenpruefung VOR allem anderen: was der Spread vom Risiko
            # frisst, muss die Strategie erst wieder aufholen.
            spread_px = datafeed.paper_spread(sym, entry)
            cost_r = spread_px / dist if dist > 0 else 99.0
            rr_used = datafeed.rr_for(sym, float(p.get("rr", 2.0)))
            cost_limit = MAX_COST_HANDICAP_PP / 100.0 * (rr_used + 1.0)
            if cost_r > cost_limit:
                log({"kind": "SKIP_COST", "ts": now_iso(),
                     "cand": cid, "symbol": sym,
                     "stop_pct": round(dist / entry * 100, 3),
                     "cost_r": round(cost_r, 3),
                     "limit_r": round(cost_limit, 4), "rr": rr_used,
                     "breakeven_winrate": round((1 + cost_r) / (rr_used + 1) * 100, 1)})
                if verbose:
                    print(f"  {sym}: uebersprungen - Kosten {cost_r:.3f}R bei {rr_used:.0f}R-Ziel "
                          f"(Stop {dist/entry*100:.2f}%), braeuchte "
                          f"{(1+cost_r)/(rr_used+1)*100:.1f}% Trefferquote")
                continue

            risk_amount = book["equity"] * RISK_PCT / 100.0
            qty = risk_amount / dist

            # Kann das Konto diese Position ueberhaupt tragen? Der Gegenwert
            # waechst, je enger der Stop sitzt - bei 0.25% Stop ist er das
            # Vierfache des Kontos. Ohne diese Pruefung eroeffnet der Bot
            # Positionen, die ein echter Broker abgelehnt haette.
            acct = account_state(book)
            lev = datafeed.leverage_of(sym)
            full_qty = qty
            need = (qty * entry) / max(1, lev)
            shrunk = False
            if need > acct["free_margin"]:
                # So gross, wie das freie Kapital es zulaesst. Das Risiko in
                # Euro sinkt entsprechend mit - R bleibt vergleichbar, weil
                # jeder Trade gegen sein EIGENES Risiko gerechnet wird.
                afford = acct["free_margin"] * lev / entry
                if afford <= 0 or afford < full_qty * MIN_SIZE_FRAC:
                    log({"kind": "SKIP_NO_MARGIN", "ts": now_iso(),
                         "cand": cid, "symbol": sym,
                         "stop_pct": round(dist / entry * 100, 3),
                         "wanted_qty": round(full_qty, 6),
                         "affordable_qty": round(max(afford, 0), 6),
                         "leverage": lev,
                         "margin_free": round(acct["free_margin"], 2)})
                    if verbose:
                        print(f"  {sym}: uebersprungen - selbst verkleinert zu klein "
                              f"(Stop nur {dist/entry*100:.2f}%)")
                    continue
                qty = afford
                shrunk = True

            notional = qty * entry
            need = notional / max(1, lev)
            risk_amount = qty * dist        # tatsaechliches Risiko nach Anpassung
            if shrunk:
                log({"kind": "SIZE_CAPPED", "ts": now_iso(),
                     "cand": cid, "symbol": sym,
                     "stop_pct": round(dist / entry * 100, 3),
                     "wanted_qty": round(full_qty, 6), "used_qty": round(qty, 6),
                     "frac": round(qty / full_qty, 3),
                     "risk_amount": round(risk_amount, 2), "leverage": lev})
                if verbose:
                    print(f"  {sym}: verkleinert auf {qty/full_qty*100:.0f}% "
                          f"(Risiko {risk_amount:,.0f} statt "
                          f"{full_qty*dist:,.0f})")
...
def report() -> None:
    ...
    from live_strategy import LIVE as CANDIDATES
