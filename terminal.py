...
def _paper_source():
    """Adresse des Bot-Geraets: Umgebungsvariable, sonst Datei.

    Die Datei ist der verlaesslichere Weg: eine vom Aufgabenplaner gestartete
    Aufgabe erbt eine mit setx gesetzte Variable nicht zuverlaessig, eine Datei
    neben dem Skript liest sie dagegen immer.
    """
    v = os.environ.get("PAPER_BOOK_URL", "").strip()
    if v:
        return v
    for name in ("paper_source.txt",):
        for base in (os.path.dirname(APP_DIR), APP_DIR):
...
def account_view():
    try:
        ...
        # Nicht stumm scheitern: ohne Grund sucht man den Fehler im Netzwerk,
        # obwohl vielleicht nur die Datei fehlt - oder umgekehrt.
        return {"present": False,
                "remote": bool(PAPER_BOOK_URL),
                "source": PAPER_BOOK_URL or PAPER_BOOK,
    ...
    for p in openp:
        ...
        notional_p = float(p.get("notional") or (qty * entry))
        lev_p = int(p.get("leverage") or 0)
        open_rows.append({
            ...
            "notional": round(notional_p, 2), "leverage": lev_p,
            "margin": round(float(p.get("margin") or 0), 2),
    ...
    return {"present": True,
            "remote": bool(PAPER_BOOK_URL),
            "remoteWarn": warn,
            # Kapitalbindung. Der Bot schreibt margin/notional beim Eroeffnen
            # mit; aeltere Positionen ohne die Felder zaehlen als 0, das
            # korrigiert sich mit jedem neuen Trade von selbst.
            "usedMargin": round(sum(float(p.get("margin") or 0) for p in openp), 2),
            "freeMargin": round(cash + unreal
                                - sum(float(p.get("margin") or 0) for p in openp), 2),
            "notional": round(sum(float(p.get("notional") or 0) for p in openp), 2),
            "start": PAPER_START,
            "cash": round(cash, 2),
